...
from agents.historical_data_fetcher import StockDataFetcher
from agents.financial_news_fetcher import FinancialNewsFetcher
from agents.manager_agent import generate_prompt,get_prediction,calculate_risk_metrics,manage_within_episode_risk,manage_over_episode_risk
...
access_token = "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"
finnhub_client = finnhub.Client(api_key="ctind51r01qgfbsv1bi0ctind51r01qgfbsv1big")
...
def predict(ticker, date, n_weeks, use_basics):
    ...
    try:
        ...
        logging.info(f"Fin GPT answer {answer}")

        # Get historical data
        logging.info("Getting historical data")
        logging.info(f"Date: {date}")
        historical_data = hist_fetcher.get_historical_data(ticker, date, n_weeks)
        print(f"Historical data: {historical_data}")

        # Get live data
        logging.info("Getting live data")
        live_data = historical_data.tail(1)

        # Calculate risk metrics
        risk_metrics = calculate_risk_metrics(live_data, historical_data)
        logging.info(f"Risk metrics: {risk_metrics}")

        # Manage within-episode risk
        within_risk = manage_within_episode_risk(live_data, risk_metrics)
        logging.info(f"Within episode risk: {within_risk}")

        # Calculate PnL and Drawdown for over-episode risk management
        current_price = live_data['Close'].iloc[-1]  # Most recent closing price
        previous_price = historical_data['Close'].iloc[-2] if len(historical_data) > 1 else current_price
        pnl = current_price - previous_price
        peak_price = historical_data['Close'].max()  # Peak closing price in the historical data
        drawdown = (peak_price - current_price) / peak_price if peak_price > 0 else 0

        # Manage over-episode risk
        over_episode_decision = manage_over_episode_risk(pnl, drawdown)
        logging.info(f"Over episode risk management: {over_episode_decision}")

        # Fetch news and make trading decision
        logging.info("Fetching news")
        news = news_fetcher.fetch_news(ticker, date, n_weeks)

        agent = DecisionAgent(sentiment_threshold=0.2)
        decision = agent.make_decision(news)
        logging.info(f"Decision: {decision}")


        # Generate final prompt
        prompt = generate_prompt(answer, historical_data, live_data, decision, risk_metrics, within_risk, over_episode_decision)
...
demo = gr.Interface(
    ...
    title="Financial LLM-Forecaster",
    description="""
